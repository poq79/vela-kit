...
type Updater func(value []byte, found bool, expireAt uint32) (newValue []byte, replace bool, expireSeconds int)
...
func (cache *Cache) Update(key []byte, updater Updater) (found bool, replaced bool, err error) {
	...
	retValue, expireAt, err := cache.segments[segID].get(key, nil, hashVal, false)
	if err == nil {
		found = true
	} else {
		err = nil // Clear ErrNotFound error since we're returning found flag
	}
	value, replaced, expireSeconds := updater(retValue, found, expireAt)
	if !replaced {
		return
	}
	err = cache.segments[segID].set(key, value, hashVal, expireSeconds)
